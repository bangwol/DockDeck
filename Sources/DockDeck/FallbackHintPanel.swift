...
enum FallbackHintPanel {
    ...
    static let markWidth: CGFloat = 32
    static let markRowGap: CGFloat = 8
    ...
    static func make(
        ...
        let markHeight = (markWidth * DockDeckMark.aspectRatio).rounded(.up)
        let height = padding + markHeight + markRowGap + textHeight + padding
        ...
        let mark = NSHostingView(rootView: DockDeckMark())
        mark.frame = NSRect(
            x: (width - markWidth) / 2, y: padding + textHeight + markRowGap,
            width: markWidth, height: markHeight)
        effectView.addSubview(mark)
