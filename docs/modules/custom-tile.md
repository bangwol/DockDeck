...
Choose a 1-, 5-, or 15-minute interval. Disabling the module cancels scheduled
and in-flight runs and discards late results. Hidden polling is three times
slower, and macOS Low Power Mode adds another two-times multiplier. `⌘R`
requests an immediate run for every enabled Custom Tile slot.
