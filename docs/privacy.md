...
| System Stats | macOS host, file-system, routing, IOKit, Network, and `ProcessInfo` APIs; optional validated local Stats SMC tool | Samples selected CPU, GPU, memory, disk, network-counter, and temperature values locally. Trends stay in memory for at most 15 minutes. |
